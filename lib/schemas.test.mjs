...
describe("CategorySchema", () => {
  ...
  it("requires isAward", () => {
    const base = { id: "cannes:palme-dor", festivalId: "cannes", name: "Palme d'Or", normalizedName: "palme-dor", scope: "film", isHonoraryAward: false };
    expect(CategorySchema.safeParse(base).success).toBe(false);
    expect(CategorySchema.safeParse({ ...base, isAward: false }).success).toBe(true);
  });

  it("requires isHonoraryAward", () => {
    const base = { id: "cannes:palme-dor", festivalId: "cannes", name: "Palme d'Or", normalizedName: "palme-dor", scope: "film", isAward: true };
    expect(CategorySchema.safeParse(base).success).toBe(false);
    expect(CategorySchema.safeParse({ ...base, isHonoraryAward: false }).success).toBe(true);
  });
});
